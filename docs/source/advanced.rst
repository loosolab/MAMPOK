...
Or use ``create-mamplan``::
    ...
    mampok create-mamplan --auth --config /path/to/config.json ...
...
To change the user list or rotate the secret key without redeploying::
    ...
    mampok update-auth my-project-mamplan.json --config /path/to/config.json -Y
...
for monitoring::
    ...
    mampok stop-expired ~/mamplans/ --config /path/to/config.json -Y || notify_on_failure
...
Use ``list-expiring`` to get warned before projects expire::
    ...
    mampok list-expiring ~/mamplans/ --within 7d --config /path/to/config.json

    # Projects expiring in the next 2 weeks
    mampok list-expiring ~/mamplans/ --within 2w --config /path/to/config.json
...
more YAML files in `FRED <https://github.com/loosolab/FRED>`_ format to
populate the service section automatically.
...
.. list-table::
   ...
   * - ``technical_details.techniques[].technique[]``
     - ``service.datatype``
   * - ``project.id``
     - ``service.metadata``
